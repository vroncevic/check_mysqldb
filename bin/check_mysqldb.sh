...
function __check_mysqldb() {
	...
	if [ -n "${DN}" ]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$CHECK_MYSQLDB_TOOL"
		__progress_bar PB_STRUCTURE
		declare -A config_check_mysqldb=()
		__load_conf "$CHECK_MYSQLDB_CFG" config_check_mysqldb
		STATUS=$?
		if [ $STATUS -eq $NOT_SUCCESS ]; then
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$CHECK_MYSQLDB_TOOL"
			exit 129
		fi
		...
		__logging CHECK_MYSQLDB_LOGGING
		__info_debug_message_end "Done" "$FUNC" "$CHECK_MYSQLDB_TOOL"
		exit 0
	fi
	__usage CHECK_MYSQLDB_USAGE
	exit 128
}
